...
export const analyzeHealthTrends = async (user, monitoringHistory) => {
  try {
    const model = bioMistralClient.getGenerativeModel({
    ...
    const dataSummary = monitoringHistory.map((entry, index) => ({
      ...
      vitals: entry.vitals
        ? `Sugar:${entry.vitals.sugar} BP:${entry.vitals.bpHigh}/${entry.vitals.bpLow} Weight:${entry.vitals.weight}kg`
        : "N/A",
...
export const generateDietRecommendations = async (
  user,
  monitoringData,
  healthAnalysis,
) => {
