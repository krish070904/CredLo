...
const getBMICategory = (bmi) => {
  ...
  if (bmi < 18.5)
    return { category: "Underweight", emoji: "⚠️", color: "yellow" };
  ...
  return {
    ...
    summary: {
      ...
      alertRate:
        raw.summary.totalEntries > 0
          ? ((raw.summary.alertCount / raw.summary.totalEntries) * 100).toFixed(
              1,
            ) + "%"
          : "0%",
    ...
    entries: Array.isArray(raw.entries)
      ? raw.entries
          .map((entry, index) => {
            ...
            const condition =
              entry.modelResult?.predicted_condition || "Not analyzed";
            return {
      ...
      ? raw.alerts
          .map((alert, index) => {
            const severityInfo = getSeverityInfo(
              alert.severityScore || alert.severity,
            );
            return {
    ...
    statistics: {
      ...
      mediumSeverityCount: Array.isArray(raw.entries)
        ? raw.entries.filter(
            (e) => (e.severity ?? 0) > 30 && (e.severity ?? 0) <= 70,
          ).length
        : 0,
